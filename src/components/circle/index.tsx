...
import { MIN_RADIUS_CIRCLE } from "@/utils/constant";
...
function useCircle(props: CircleProps) {
  ...
  useEffect(() => {
    ...
    gme.addListener(circle, "radius_changed", () => {
      ...
      if (newRadius < MIN_RADIUS_CIRCLE) {
        circle.setRadius(MIN_RADIUS_CIRCLE);
        callbacks.current.onRadiusChanged?.(MIN_RADIUS_CIRCLE);
      } else callbacks.current.onRadiusChanged?.(newRadius);
    });
